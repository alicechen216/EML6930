...
C = [1 1 1];                        % Output matrix (observing the first state only)
...
% Time-invariant uncertainties for the first state of each follower
w = [0.5; 1; -0.5; -1];       % Uncertainties for followers 2 to 5
...
for k = 2:num_steps
    ...
    % Update each follower's state based on system dynamics and uncertainty
    x_dot_followers = zeros(size(x_followers));
    for i = 1:num_agents - 1
        % Time-invariant uncertainty affecting the first state
        w_i = [w(i); 0; 0];
        
        % Compute the state derivative with uncertainty
        x_dot_followers(i, :) = (A * x_followers(i, :)') + B * u(i) + w_i;
    end
    
    % Update follower states
    x_followers = x_followers + time_step * x_dot_followers;
