% MATLAB script for consensus protocol with Brownian noise and barrier function in adaptive bias estimation
clc; clear;
...
noise_std = 0.01;
% Consensus protocol update loop with Brownian noise and adaptive bias with barrier function
for k = 2:num_steps
...
% Set threshold for norm constraint
delta = 0.001;       % Maximum allowable norm deviation between each bias and leader
b_leader = 0;       % Assuming the leader’s bias is zero

% Adaptive law with norm constraint barrier function
for i = 1:num_agents - 1
    ...
    % Consensus term with bias compensation and norm constraint
    consensus_term = 0;
    for j = 1:num_agents - 1
        if L(i+1, j+1) == 1  % Only consider neighbors
            % Calculate the norm-based barrier function gradient
            norm_diff = norm(bias_estimates(i, j, :) - b_leader);
            if norm_diff < delta
                barrier_grad = 0;  % No barrier effect within safe set
            else
                % Apply barrier gradient as bias estimate approaches delta boundary
                barrier_grad = (bias_estimates(i, j, :) - b_leader) / (delta^2 - norm_diff^2);
                
                %barrier_grad = (bias_estimates(i, j, :) * b_max^2) ./ (b_max^2 - bias_estimates(i, j, :).^2).^2;

            end
            
            % Update bias estimates adaptively with leakage and barrier function gradient
