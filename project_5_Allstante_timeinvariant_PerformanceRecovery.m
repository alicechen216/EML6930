...
B = eye(3);                         % Input matrix (identity since control input is vectorial)
C = [1 0 0];                        % Output matrix (observing the first state only)

% LQR cost matrices
Q = 0.1*eye(3);                         % State-cost matrix
R = 0.01 * eye(3);                  % Input-cost matrix (adjusted for vector input)
...
gamma = 1;                          % Assuming gamma = 1 (not specified)
...
K_array = zeros(num_agents - 1, 3, 3); % LQR gains for each follower (3x3 matrices)
for i = 2:num_agents                % Skip lambda_1 = 0 (consensus mode)
    lambda_i = sqrt(lambda(i));
    K_i = lqr(A, lambda_i * B, Q, R); % Compute LQR gain for each follower
    K_array(i-1, :, :) = K_i;          % Store the gain matrix
end
...
delta_followers = [5  -7  -3;
                   4  -5  -4;
                   6   3  -5;
                  -4   1   2]';     % Each column corresponds to a follower's delta_i (3x4 matrix)
...
for k = 2:num_steps
    ...
    for i = 1:num_agents - 1
        ...
        % Scaling consensus error by c (consensus scaling factor)
        consensus_error = c * consensus_error;
        
        % Compute the total error
        e_total = e_i + consensus_error;
        
        % Compute the corrective signal
        corrective_signal = - (gamma * A' * P + mu * eye(3)) * delta_followers(:, i);
        
        % Compute the control input for follower i
        u(:, i) = -K_i * e_total + corrective_signal;
    end
    
    % Update each follower's state based on system dynamics with uncertainties
    x_dot_followers = (A * x_followers') + u + delta_followers; % Add uncertainties
