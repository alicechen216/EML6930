% MATLAB script for consensus protocol with noise and adaptive bias estimation
clc; clear;
...
% Consensus protocol update loop with noise and adaptive bias compensation
for k = 2:num_steps
    ...
    noise = noise_std * randn(size(y_followers));  % Gaussian noise for each follower measurement
    y_followers_noisy = y_followers + noise;      % Add noise to the followers' measurements
    
    % Store noise data for plotting
    noise_history(:, k) = noise(:, 1);
        ...
        for j = 1:num_agents - 1
            if L(i+1, j+1) == 1  % Only consider neighbors
                ...
                bias_estimates(i, j, :) = bias_estimates(i, j, :) + ...
                    gamma * ((y_followers_noisy(j) - y_followers_noisy(i)) - bias_estimates(i, j, :)) * time_step;
...
title('Noise Effect on Followers over Time','FontSize',24);
xlabel('Time');
