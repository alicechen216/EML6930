...
num_followers = num_agents - 1;     % Number of followers
...
K_array = zeros(num_followers, 3);  % LQR gains for each follower (row per agent)
for i = 2:num_agents                 % Skip lambda(1) = 0 (as it corresponds to the consensus mode)
    lambda_i = sqrt(lambda(i));
    % Adjust the cost matrices for each agent
    Q_i = lambda_i * Q;
    R_i = R;
    % Compute LQR gain for each follower
    [K_i, ~, ~] = lqr(A, B, Q_i, R_i);
    K_array(i-1, :) = K_i;
end
...
bias_estimates = zeros(num_followers, num_followers, 3);  % Bias estimate is a vector for each follower pair

% Store the history of states and errors for plotting
x_history = zeros(num_agents, 3, num_steps);
error_history = zeros(num_followers, num_steps); % Error between followers and the leader
noise_history = zeros(num_followers, num_steps); % Noise added to each follower

% Initial state of the reference model (Agent 1)
x_ref = [1; 1; 3];                   % Initial state of the reference model for Agent 1

% Initial states for followers (each row corresponds to a follower)
x_followers = [0, 0, 0;
               1, 2, -1;
               3, 1, 0;
               2, 2, 2];  % Followers' initial states
...
constant_noise = [5, -7, -3;
                  4, -5, 4;
                 -6,  3, -5;
                 -4, -1, -2];  % Constant noise for each agent's state

% Consensus protocol update loop with constant noise across all states and adaptive bias compensation
for k = 2:num_steps
    % Update Agent 1 (reference model) with the dynamics
    x_dot_ref = A * x_ref;                   % Compute the derivative
    x_ref = x_ref + x_dot_ref * time_step;   % Update Agent 1's state
    ...
    for i = 1:num_followers
        ...
        % Compute the consensus term with bias compensation
        consensus_term = zeros(3, 1);
        for j = 1:num_followers
            if A_adj(i+1, j+1) == 1  % Only consider neighbors (offset by 1 due to leader)
                % Update bias estimates adaptively
                delta = (x_followers_noisy(j, :)' - x_followers_noisy(i, :)') - squeeze(bias_estimates(i, j, :));
                bias_estimates(i, j, :) = squeeze(bias_estimates(i, j, :)) + gamma * delta * time_step;

                % Consensus term with adaptive bias compensation
                consensus_term = consensus_term + delta;
            end
        end

        % Total control input for follower i
        u(i) = -K_i * (x_followers_noisy(i, :)' - x_ref) - K_i * consensus_term;
    end

    % Update each follower's state based on system dynamics
    x_dot_followers = zeros(num_followers, 3);
    for i = 1:num_followers
        x_dot_followers(i, :) = (A * x_followers(i, :)')' + (B * u(i))';  % Update follower i
    end
    x_followers = x_followers + x_dot_followers * time_step;  % Update follower states
    ...
    error_history(:, k) = vecnorm(x_followers' - x_ref, 2, 1)';  % Error between each follower and the leader
end
...
colors = lines(num_followers);  % Use distinct colors for followers
for i = 2:num_agents
